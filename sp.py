...
def repeat_experiment(network_type, train_loader, test_loader, device, seeds, lrs, every_batches, threshold_levels, alpha_perturbs, noise_stds, threshold_norm_levels):

    print(f"Training {len(seeds)}x{len(lrs)}x[1+{len(every_batches)}x{len(threshold_levels)}x{len(alpha_perturbs)}x{len(noise_stds)}x{len(threshold_norm_levels)}] models of {network_type}")
...
def setup_wandb(config):
    ...
    run = wandb.init(project=wandb_project, entity=wandb_entity, config=config, 
                     dir="/home/mateuszpyla/stan/sharpness/sp_results", 
...
def main(network_type, seeds, lrs, every_batches):
    ...
    R = repeat_experiment(
        train_loader, test_loader, device, network_type, \
        seeds, lrs, every_batches, \
        threshold_levels=[0.01], #, 0.5, 1.0], \
...
if __name__ == "__main__":
    ...
    parser.add_argument('network_type', type=str, choices=['fc', 'lenet'], default='fc')
    ...
    main(args.network_type, seeds, lrs, every_batches)
